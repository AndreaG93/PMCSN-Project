package nexteventsimulation.utility.API;/* -------------------------------------------------------------------------
 ...
 */
    ...
    if (x < 0) {
      Date now = new Date();
      x = now.getTime();
//      x = ((unsigned long) startTime((time_t *) NULL)) % MODULUS;
    }
    if (x == 0)
      while (!ok) {
        try {
...
/* ------------------------------------------------------------------
 * Use this (optional) function to test for a correct implementation_fifo.
 * ------------------------------------------------------------------
 */
